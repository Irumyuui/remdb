...
const HEADER_SIZE: usize = mem::size_of::<u32>() + mem::size_of::<u32>() + mem::size_of::<u32>(); // + mem::size_of::<u8>();
...
impl Wal {
    ...
    fn pending_zero_next_entry(&mut self) -> Result<()> {
        let zero_header =
            &mut self.mmap[self.write_offset as usize..self.write_offset as usize + HEADER_SIZE];
        unsafe {
    ...
    pub fn put(&mut self, key: &[u8], value: &[u8]) -> Result<()> {
        self.put_batch([(key, value)].iter().cloned())
    }

    pub fn put_batch<'a>(
        &'a mut self,
        data: impl Iterator<Item = (&'a [u8], &'a [u8])>,
    ) -> Result<()> {
        for (k, v) in data {
            Self::reload_entry(&mut self.buf, k, v);
    ...
    fn reload_entry(buf: &mut Vec<u8>, key: &[u8], value: &[u8]) -> Result<()> {
        buf.clear();

        buf.resize(HEADER_SIZE, 0);
        buf.reserve(HEADER_SIZE + key.len() + value.len());
...
mod tests {
    ...
    fn write_some_data() -> anyhow::Result<()> {
        ...
        wal.put(b"hello", b"world")?;
        wal.finish()?;
        drop(wal);

        let mut expected: Vec<u8> = vec![0; HEADER_SIZE + 5 + 5];
        expected[4..].as_mut().put_u32_le(5);
        expected[8..].as_mut().put_u32_le(5);
        expected[12..].copy_from_slice(b"hello");
        expected[17..].copy_from_slice(b"world");
        let c = crc32fast::hash(&expected[4..]);
        expected[..].as_mut().put_u32_le(c);

        let mut file = std::fs::OpenOptions::new().read(true).open(&test_path)?;
        let mut actual = vec![0; HEADER_SIZE + 5 + 5];
