...
use super::mem_alloc::MemAllocator;
...
impl<K, V, A> Default for SkipList<K, V, A>
where
    K: Ord,
    ...
    A: MemAllocator,
{
    pub fn new(allocator: A) -> Self {
        unsafe {
            let head = Node::<K, V>::new_head(&allocator, MAX_HEIGHT);
    ...
    fn find_greater_or_equal(
        &self,
        key: &K,
        mut prev: Option<&mut [*mut Node<K, V>]>,
    ) -> *mut Node<K, V> {
        unsafe {
...
impl<K, V, A> Debug for SkipList<K, V, A>
where
    K: Ord + Debug,
    V: Debug,
    A: MemAllocator,
...
pub struct SklIter<K: Ord, V, A: MemAllocator> {
    skl: Arc<SkipList<K, V, A>>,
    node: *const Node<K, V>,
}

impl<K, V, A> SklIter<K, V, A>
where
    K: Ord,
    A: MemAllocator,
{
    pub fn new(skl: Arc<SkipList<K, V, A>>) -> Self {
        Self {
            skl,
            node: std::ptr::null(),
        }
...
mod tests {
    ...
    use crate::utils::{mem_alloc::DefaultAllocator, skip_list::SklIter};

    use super::SkipList;

    #[test]
    fn insert_some() {
        let list: SkipList<i32, (), DefaultAllocator> = SkipList::new(DefaultAllocator::default());
        for i in 0..1000 {
            list.insert(i);
        }
        for i in 0..1000 {
